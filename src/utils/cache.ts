...
const { VITE_PREFIX, VITE_IS_ENCRYPT, VITE_SECRET_KEY, VITE_SECRET_VI } =
  import.meta.env

const prefix = VITE_PREFIX + '-' + version + '-'
...
export const local = {
  ...
  set(key: string, value: any, expire?: TimeType): void {
    ...
    if (key !== null && value !== null) {
      const data = {
        value: VITE_IS_ENCRYPT
          ? encryptData(value, VITE_SECRET_KEY, VITE_SECRET_VI)
  ...
  get(key: string): string {
    ...
    if (key && localStorage) {
      ...
      if (data !== null) {
        ...
        if (VITE_IS_ENCRYPT) {
          value = decryptData(value, VITE_SECRET_KEY, VITE_SECRET_VI)
        }
      }
    }
